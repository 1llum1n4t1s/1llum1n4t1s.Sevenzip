...
## [1.0.90] — Git 記録日: 2026-09-19

- **破壊的変更:** `SuperLightLogger` 依存、`Cube.Logging` プロジェクト、`Cube.Logger` / `ILoggerSource` / `LogLevel` API を削除しました。ライブラリは独自にログを出力せず、処理失敗を例外として呼び出し元へ返します。
- 従来は警告して無視していた未対応オプションと、ログだけに記録していた I/O・フラッシュ・破棄失敗を例外で通知するようにしました。意図的なアクセス不能ファイルのスキップは引き続き `FileSkipped` イベントで通知します。
- COM コールバックとストリーム処理で発生した元の例外を保持し、複数の後始末に失敗した場合も例外を集約して通知するようにしました。
- アーカイブ更新・TAR 出力・分割ボリューム作成の上書き、ディスク同期、早期 EOF 処理を改善し、不完全な出力や無限ループを防止しました。
- 空 ZIP、ZIP64、分割 ZIP と短いストリーム読み取りの形式判定を改善し、任意の `0x78` 先頭データを DMG と誤認しないようにしました。
- Stream から分割書庫を開く場合の `sourceHint` 契約を明確化し、追加ボリュームを解決できない原因を例外として通知するようにしました。
- 7-Zip COM のプロパティ設定結果とアーカイブ項目数の境界を検証し、不正な設定や扱えない項目数を明示的に拒否するようにしました。

## [1.0.89] — Git 記録日: 2026-09-07

- 同梱の 7-Zip を 26.03 に更新し、本家の不具合・脆弱性修正を取り込みました。
- NuGet パッケージに 7-Zip のライセンス情報を同梱しました。
